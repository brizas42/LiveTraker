            ...
            <div class="glass dark:bg-gray-800/80 dark:border-gray-700 p-8 rounded-3xl shadow-sm relative overflow-hidden">
                <form action="{{ route('habits.update', $habit) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" required value="{{ old('name', $habit->name) }}" class="block w-full rounded-xl border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white/50 dark:bg-gray-700/50 dark:text-white" placeholder="Ex: Boire 2L d'eau, Lire 15 mins...">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3" class="block w-full rounded-xl border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white/50 dark:bg-gray-700/50 dark:text-white" placeholder="Pourquoi voulez-vous instaurer cette habitude ?">{{ old('description', $habit->description) }}</textarea>
                    </div>
                    ...
                    <div class="bg-indigo-50 dark:bg-indigo-900/20 p-6 rounded-2xl border border-indigo-100 dark:border-indigo-800/50">
                        ...
                        <input type="time" name="reminder_time" value="{{ old('reminder_time', $habit->reminder_time ? \Carbon\Carbon::parse($habit->reminder_time)->format('H:i') : null) }}" class="block w-full md:w-1/2 rounded-xl border-indigo-200 dark:border-indigo-800 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white dark:bg-gray-800 text-indigo-900 dark:text-indigo-100 font-bold text-lg p-3">
                    </div>
